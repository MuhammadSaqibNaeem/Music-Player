...
export default class AudioPlayer extends React.Component {
  ...
  async componentDidMount() {
    ...
    const likedItems =
      JSON.parse(await AsyncStorage.getItem("likedItems")) || [];
    this.setState({
      isLiked: likedItems.some((likedItem) => likedItem.id === item.id),
    });

    await this._loadNewPlaybackInstance(item.preview);
          ...
          <View style={styles.heratTitleView}>
            <View style={styles.imgTitle}>
              <Text style={styles.songName}>
                {item?.item?.title ? item?.item?.title : "Ariana Grande"}
              </Text>
              <Text style={styles.songDesc}>
                {item?.item?.artist?.name
                  ? item?.item?.artist?.name
                  : "The Weekend . Daft bunk"}
              </Text>
            </View>
